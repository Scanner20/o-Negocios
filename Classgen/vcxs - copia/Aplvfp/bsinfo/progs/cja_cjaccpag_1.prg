...
DIMENSION vDatos(20,6),vCdCta(20)
STORE '' TO vDatos
DO Carga0
IF NumEle = 0
...
XiForma  = 1
FCH = DTOC(XdFchVto)
DIMENSION vDatos(20,6),vCdCta(20,2)
DO CARGA0
...
PROCEDURE xCstMov2
******************
PARAMETERS XsProveedor,XsCuentas,XsTpoDoc
MaxEle = 10
...
ZfSalAct1 = 0
ZfSalAct2 = 0
*** Creamos un vector con la cadena de cuentas para evaluarlas individualmente ***
LOCAL ARRAY LaCuentas[1]
LcDelimitador=''
LnDelimitador=AT(',',XsCuentas)
IF LnDelimitador>0
	LcDelimitador=','
ELSE	
	IF AT(';',XsCuentas)>0
...
IF !EMPTY(LcDelimitador) 
	...
	IF !EMPTY( XsCuentas )
		ChrToArray( XsCuentas, LcDelimitador , @LaCuentas )
	ENDIF
ELSE && es solo una sola serie
	LaCuentas[1] = TRIM(PADR(XsCuentas,LEN(CTAS.Codcta)))
ENDIF

SELECT  CTAS 
*!*	SEEK [42]
*!*	DO WHILE CodCta=[42] .AND. ! EOF() && trim(CodCta)$XsCuentas
FOR j= 1 TO ALEN(LaCuentas)
	SEEK LaCuentas[J]
	SCAN WHILE CodCta=LaCuentas[J]
...
USE (LsCursorA) ALIAS Cur_DetDoc
FOR K = 1 TO NumEle && ALEN(vDatos,1)
	IF vDocum(k,6)<>0 OR vDocum(k,7)<>0
	APPEND BLANK
		REPLACE CodCta WITH vDocum(k,1)
		REPLACE NroDoc WITH vDocum(k,2)
		REPLACE NroRef WITH vDocum(k,3)
...
DO WHILE ClfAux = XsClfAux .AND. ! EOF()
	...
	LsCodAux = CodAux
	LsNomAux = NomAux
	STORE 0 TO ZfTotMn,ZfTotMe,ZfSalAct1,ZfSalAct2   
	LiNroItm = 0
	FOR Item = 1 TO NumCta
		LsCodCta = vCodCta(Item,1)
		SELECT RMOV
		...
		DO WHILE CodCta+CodAux = LsCodCta+LsCodAux .AND. ! EOF()
			...
			DO WHILE CodCta+CodAux+NroDoc = LsCodCta+LsCodAux+LsNroDoc .AND. ! EOF()
				...
				IF TpoMov = "H"
					SalAct1 = SalAct1 + IMPORT
					SalAct2 = SalAct2 + IMPUSA
					**>>  Vo.Bo. VETT  2008/11/14 13:31:24 
					ZfTotMN = ZfTotMN + IMPORT/(1+gosvrcbd.xfporigv/100)  && Solo la deuda contraida VETT 2008-06-04
					ZfTotME = ZfTotME + IMPUSA/(1+gosvrcbd.xfporigv/100)  && Solo la deuda contraida VETT 2008-06-04
					**>>  Vo.Bo. VETT  2008/11/14 13:31:47 
				ELSE
					SalAct1 = SalAct1 - IMPORT
					SalAct2 = SalAct2 - IMPUSA
					ZfTotMN = ZfTotMN - IMPORT/(1+gosvrcbd.xfporigv/100)  && Solo la deuda contraida VETT 2008-06-04
	...
	IF ZfSalAct1#0 .OR. ZfSalAct2#0
		...
		IF XiForma = 0
			...
			ELSE
				vDatos(NumEle,4)   =  ZfSalAct2 		&& TRANSF(-ZfSalAct2,"@Z 9,999,999.99- ")
			ENDIF
			vDatos(NumEle,5)	= ZfTotMN
			vDatos(NumEle,6)	= ZfTotME
		ENDIF
	ENDIF
	SELECT AUXI
	SKIP
...
DO CASE
	CASE XiForma=0
		...
		ENDIF
		SELECT 0
		CREATE TABLE (LsCursor) FREE (Codigo C(11), Nombre C(50), SaldoMN n(14,2),SaldoME N(14,2),TotMN n(14,2),TotME n(14,2))
		USE
		USE (LsCursor) ALIAS Cur_Saldo EXCLUSIVE
		INDEX on Codigo TAG Codigo DESCENDING
		INDEX on Nombre TAG Nombre DESCENDING
		INDEX on SaldoMn TAG SaldoMn DESCENDING
		INDEX on SaldoMe TAG SaldoMe DESCENDING
		INDEX on TotMN TAG TOTMN DESCENDING
		INDEX on TotME TAG TOTME DESCENDING
		...
		FOR K = 1 TO NumEle && ALEN(vDatos,1)
			...
			REPLACE TOTMN WITH IIF(EMPTY(vDatos(k,5)),0.00,vDatos(k,5))
			REPLACE TOTME WITH IIF(EMPTY(vDatos(k,6)),0.00,vdatos(k,6))
...
*!*		@ 24,0 SAY PADC(TRIM(LsCodAux)+" "+TRIM(LsNomAux),80)
	...
	DO WHILE CodCta+CodAux = LsCodCta2+LsCodAux .AND. ! EOF()
		SalAct1  = 0
		SalAct2  = 0
		=SEEK(NROMES+CODOPE+NROAST,"VMOV")
		LsNroDoc = NroDoc
		...
		LsGloDoc = GloDoc
		LsNotAst = VMOV->NotAst
		DO WHILE CodCta+CodAux+NroDoc = LsCodCta2+LsCodAux+LsNroDoc .AND. ! EOF()
			IF TpoMov = "H"
				SalAct1 = SalAct1 + IMPORT
				SalAct2 = SalAct2 + IMPUSA
			ELSE
