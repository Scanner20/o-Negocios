...
PUBLIC k_home,k_end,k_pgup,k_pgdn,k_del,k_ins,k_f_izq,k_f_arr,k_f_aba,k_f_der,;
   ...
   k_ctrlw,k_lookup,k_borrar,k_esc,SERR
...
STORE "" TO _Prn0,_Prn1,_Prn2,_Prn3,_Prn4,_Prn5a,_Prn5b,_Prn6a,_Prn6b,;
            _Prn7a,_Prn7b,_Prn8a,_Prn8b,_Prn9a,_Prn9b,_Prn10a,_prn10b,SERR
*------------------------------------------------------------------------
Dimension VecOpc(10)  && ELIGE
*------------------------------------------------------------------------
SET SYSMENU TO
SET SYSMENU ON
...
m.libfile = m.pathlib+"belcsoft.fxp"
if !file(m.libfile)
	m.LibFile=Locfile("belcsoft.fxp")
ENDIF
IF EMPTY(m.libfile)
	WAIT WINDOW "No existe libreria principal...Reinstale el sistema..Pulse una tecla" 
	quit
ENDIF
...
***** Borrando Archivo Temporales *******
CLEAR
*@ 13,0 SAY PADC("** Borrando Archivos Temporales **",80)
...
**ON ERROR DO Error WITH ERROR(),MESSAGE(),SYS(16),MESSAGE(1)
...
IF ! EMPTY(GsTerminal)
   ...
   IF ! FILE(ArcUsr+".dbf")
      ...
      IF ! FILE(ArcUsr1+".dbf")
         GsMsgErr = "Archivo : "+ArcUsr1+".dbf no registrado"
         DO LIB_MERR WITH 99
         CLOSE DATA
         QUIT
         RETURN
...
ENDIF
IF ! FILE(ArcUsr+".dbf")
   GsMsgErr = "Archivo : "+ArcUsr+".dbf no registrado"
   DO LIB_MERR WITH 99
   CLOSE DATA
   QUIT
   RETURN
...
IF EMPTY(GsUsuario)
   @ 0,11 GET GsUsuario PICTURE "@!" COLOR SCHEME 7
   @ 1,11 GET GsClave   PICTURE "@!" COLOR ,X
   READ VALID _qan0y6g9c()
ELSE
...
DO WHILE (.t.)
   ...
   CLEAR WINDOW
   DO ADMPINTA
   USE ADMMCIAS IN 3 ORDER MCIA01
      ** SELECCIONAR COMPA¥IAS **
   SELECT admmcias
   IF RECCOUNT() = 0
      CLEAR
      WAIT "No Existen compa¤ias inscritas" WINDOWS
      SET DEFAULT TO (PathDef)
      QUIT
   ENDIF
   IF RECCOUNT() > 1
      Titulo = [COD.  COMPA¥IA]
      LinReg = [CodCia+" "+NomCia]
      Ancho  = LEN( &LinReg )+2
      Xo     = 78 - Ancho
      do Busca WITH "","","",LinReg,Titulo,9,Xo,13,Ancho,""
      IF LASTKEY() = 27
        SET DEFAULT TO (PathDef)
        IF GlUnico
           QUIT
   ...
   @ 17,16 SAY "Compa¤ia : "
   @ 17,28 SAY GsCodCia+" " + GsNomCia COLOR SCHEME 7
   @ 16,16 SAY "Modulo   : "
 **@ 16,28 SAY M.desmod COLOR SCHEME 7
   TsPathDbf  = PathDef+"\"+TsPathCia
   SET DEFAULT TO (TsPathDbf)
   RELEASE ALL LIKE T*
	*** SELECCION DE MODULOS **
   USE ADMODULO IN 2 ORDER MODU01
   USE ADMRUSER IN 1 ORDER RUSE01
   SELECT admruser
   SET RELATION TO MODULO INTO admodulo
   SELECT admruser
   GlUnico = .T.
   IF ! SEEK(GsUsuario,'admruser')
      CLEAR
      ?? CHR(7)
      WAIT "No Existen modulos asignados al usuario" WINDOWS
      SET DEFAULT TO (PathDef)
      QUIT
   ENDIF
   SKIP
   IF GsUsuario = admruser->login
      Dimension vModulo(20),vCodMod(20)
      GlUnico = .F.
      MaxEle  = 0
      SEEK GsUsuario
      Ancho  = LEN(admodulo->DesMod)+2
      Xo     = 78 - Ancho
      DO WHILE Login = GsUsuario .AND. ! EOF()
         MaxEle = MaxEle + 1
         IF MaxEle > ALEN(vModulo)
            Dimension vModulo(MaxEle + 10),vCodMod(MaxEle + 10)
         ENDIF
         vModulo(MaxEle) = admodulo->DesMod
         vCodMod(MaxEle)  = Modulo
         SKIP
      ENDDO
      MaxEle = MaxEle + 1
      IF MaxEle > ALEN(vModulo)
   ...
   ELSE
     SKIP -1
   ENDIF
   M.DesMod =  admodulo->desmod
   IF "\<"$M.DesMod
      M.DesMod= PADR(STRTRAN(M.DesMod,"\<"),LEN(M.DesMod))
   ENDIF
   @ 16,16 SAY "Modulo   : "
   @ 16,28 SAY m.desmod COLOR SCHEME 7

   IF ! EMPTY(admodulo->PATHMod)
       PathOrg = PathOrg1+";"+ALLTRIM(admodulo->PATHMod)
       SET PATH TO 
       SET PATH TO (PathOrg)
   ENDIF

   GsModulo = Modulo
   GsTipo = admruser.Tipo
   GsCategoria = admruser.Categoria
   m.Master = GsCategoria
   DIMENSION A_Tipo[LEN(GsTipo)]
   FOR i = 1 TO LEN(GsTipo)
     A_Tipo[i] = SUBSTR(GsTipo,i,1)="X"
   NEXT
   IF m.master
      store .T. TO a_tipo
   ENDIF
   Program = GsMODULO+"m0000"
   CLOSE DATA
   DO (Program)
   SET DEFAULT TO (PathDef)
   IF GlUnico
      ...
      QUIT
   ENDIF
   SET PATH TO (PathOrg1)
ENDDO
RETURN

*       ÖÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ·
*       º Procedure AdmPinta                                      º
*       ÓÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ½
*
PROCEDURE ADMPINTA
RELEASE ALL WINDOWS
...
****** CHEQUEAMOS LAS COPIAS ILEGALES DEL SISTEMA ******
CLOSE ALL
IF EMPTY(GsTerminal)
   if ! FILE("\INTEGRAL.SYS")
      Integra = FCREATE("\INTEGRAL.SYS")
      IF Integra <= 0
         GsMsgErr = "No puede crear archivo \Integral.sys "
         DO LIB_MERR WITH 99
         QUIT
      ENDIF
      DATO = "                         "+CHR(13)+CHR(10)+CHR(26)
      DATO = FWRITE(INTEGRA,DATO)
   ELSE
      Integra=fopen("\INTEGRAL.SYS",2)
   ENDIF
   IF Integra <= 0
      GsMsgErr = "No tiene atributo de escritura el Archivo \Integral.sys "
      DO LIB_MERR WITH 99
      QUIT
   ENDIF
   =FCHSIZE(INTEGRA,1073)
   =FSEEK(INTEGRA,1069,0)
   DATO = FREAD(INTEGRA,4)
   =FCHSIZE(INTEGRA,1069)
   IF ! DATO == CHR(255)+CHR(255)+CHR(0)+CHR(0)
     *IF ! FILE("INFOLOCK.COM")
     *   GsMsgErr = "Faltan Archivos de Configuraci¢n"
     *   DO LIB_MERR WITH 99
     *   QUIT
     *ENDIF
     *load infolock.com
     ...
     *release module infolock
      ...
      DO CASE
         CASE A="1"
            =FSEEK(INTEGRA,1069,0)
            DATO = CHR(255)+CHR(255)+CHR(0)+CHR(0)
            DATO = FWRITE(INTEGRA,DATO)
            =FCHSIZE(INTEGRA,1069)
     *   CASE A="2"
     *     DO COPIA_ILEGAL
   ...
   ENDIF
ELSE
   USE TERMINAL.FXP ALIAS TERM EXCL
   IF ! USED()
      QUIT
   ENDIF
   IF RECCOUNT() < 1
      APPEND BLANK
   ENDIF
   xTerminal = SYS(2007,GsTerminal)
   xTerminal = PADR(GsTerminal+ENCRIPTA(xTerminal,VAL(xTerminal)),LEN(Terminal))
   LOCATE FOR TERMINAL=ENCRIPTA(xTerminal,RECNO()) .AND. RECNO()>1
   IF ! FOUND()
      GOTO 1
      IF ! RLOCK()
         GsMsgErr = "Existe otro terminal Configurandose"
         DO LIB_MERR WITH 99
         QUIT
      ENDIF
      REPLACE  TERMINAL WITH ENCRIPTA(xTerminal,RECNO())
      FLUSH
      GsMsgErr = "Terminal no inscrito en el sistema"
      DO LIB_MERR WITH 99
      LOCATE FOR TERMINAL=ENCRIPTA(xTerminal,RECNO()) .AND. RECNO()>1
      IF ! FOUND()
        *IF ! FILE("INFOLOCK.COM")
        ...
        *release module infolock
         ...
         DO CASE
            CASE A="1"
            CASE A="2"
              *Usuario presiono Escape, o el # de vidas caduco
              GOTO 1
              IF RLOCK()
...
ENDIF
CLOSE ALL
RELEASE INTEGRA,DATO,xTerminal
@  4,26 SAY "ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿" COLOR SCHEME 7
@  5,26 SAY "³         ÜÛÛ                                 ³" COLOR SCHEME 7
@  6,26 SAY "³       ÜÛÛÛÛ                                 ³" COLOR SCHEME 7
@  7,26 SAY "³     ÜÛÛß ÛÛ D M I N I S T R A D O R         ³" COLOR SCHEME 7
@  8,26 SAY "³   ÜÛÛÛÛÛÛÛÛ                                 ³" COLOR SCHEME 7
@  9,26 SAY "³ ÜÛÛß     ÛÛ          USUARIOS DEL SISTEMA   ³" COLOR SCHEME 7
@ 10,26 SAY "ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ" COLOR SCHEME 7
@ 11,26 SAY "                                               "
IF ! EMPTY(GsTerminal)
  @ 12,30 SAY "Terminal : "+GsTerminal
ENDIF
...
*       ÖÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ·
*       º                                                         º
*       º _QAN0Y6G9C           Read Level Valid                   º
*       º                                                         º
*       º Function Origin:                                        º
*       º                                                         º
...
*       ÓÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ½
*
FUNCTION _qan0y6g9c     && Read Level Valid
*
* Valid Code from screen: ALMMUSER
*
#REGION
PRIVATE i, x, c, a, b
IF LASTKEY() =  27
...
IF FOUND()
   ...
   IF b = admmuser.Password
      RETURN .T.
   ENDIF
ENDIF
?? CHR(7)
WAIT WINDOW "** Invalido Ingreso **" NOWAIT
GsClave = SPACE(LEN(admmuser.Password))
TnVeces = TnVeces - 1
IF TnVeces < 1
   QUIT
ENDIF
...
IF .NOT. TYPE("PrgFin")="C"
   ...
   PrgFin = []
ENDIF
IF .NOT. TYPE('nColor_Sch')  = "N"
	nColor_Sch = 16
ENDIF
* Dibuja el Marco  -------------------------------------------------------
@ Yo+1,Xo+1 CLEAR TO Yo+Largo-2,Xo+Ancho-2
@ Yo+1,Xo+1 FILL  TO Yo+Largo-2,Xo+Ancho-2 COLOR SCHEME  (nColor_Sch)
DO CASE
   CASE TBorde = 0
   CASE TBorde = 2
...
*!*	      SET COLOR OF SCHEME 24 TO SCHEME 11
*!*	      SET COLOR OF SCHEME 11  TO SCHEME 17
      DO &EscLin WITH EleAct, LinAct
*!*	      SET COLOR OF SCHEME 11  TO SCHEME 24
...
**      SET STEP ON 
*!*	      SET COLOR OF SCHEME 24 TO SCHEME 16
*!*	      SET COLOR OF SCHEME 16  TO SCHEME 17
      LsColorAct=SET("Color")
 *     SET COLOR OF scheme 24 TO SCHEME 16
      SET COLOR TO RGB(255,255,255,0,0,255) 	 
      DO &EscLin WITH EleAct, LinAct
      SET COLOR TO RGB(0,0,0,106,155,227),RGB(0,0,0,192,192,225),RGB(0,0,0,230,255,255),RGB(0,255,0,106,155,227),RGB(223,255,255,0,0,255),RGB(0,0,0,230,255,255),RGB(223,255,255,0,0,255),RGB(0,0,0,106,155,227),RGB(0,0,0,106,155,227),RGB(0,0,0,255,255,255),RGB(0,155,227,0,255,0)
*      SET COLOR OF scheme 16 TO SCHEME 24
...
IF _WINDOWS OR _MAC
	@ WROWS()-3,0 CLEAR TO WROWS()-1,WCOLS()
...
IF Set_Pinta
   IF VSombra .AND. Xo > 0
      @ Yo+1,Xo-1,Yo+Largo,Xo+Ancho-2 BOX REPLICATE("°",8) COLOR SCHEME (nColor_Sch)
   ...
   DO CASE
      CASE TBorde = 0
      CASE TBorde = 2
         @ Yo,Xo TO Yo+Largo-1,Xo+Ancho-1 DOUBLE COLOR SCHEME  (nColor_Sch)
      CASE TBorde = 3
         @ Yo,Xo TO Yo+Largo-1,Xo+Ancho-1 PANEL  COLOR SCHEME  (nColor_Sch)
      OTHER
         @ Yo,Xo TO Yo+Largo-1,Xo+Ancho-1  COLOR SCHEME  (nColor_Sch)
   ENDCASE
   IF ! Titulo == ""
      Col = INT((Ancho-2 - LEN(Titulo))/2)
      Col = iif(Col>0,Col,0)
...
IF Lin > 0
   IF Set_Pinta
      DO CASE
         CASE TBorde = 0
         CASE TBorde = 2
            @ Yo+1,Xo         SAY "Ç"  COLOR SCHEME  (nColor_Sch)
            @ Yo+1,Xo+1 TO Yo+1,Xo+Ancho-2 COLOR SCHEME  (nColor_Sch)
            @ Yo+1,Xo+Ancho-1 SAY "¶" COLOR SCHEME  (nColor_Sch)
         CASE TBorde = 3
            @ Yo+1,Xo TO Yo+1,Xo+Ancho-1 PANEL COLOR SCHEME  (nColor_Sch)
         OTHER
            @ Yo+1,Xo         SAY "Ã" COLOR SCHEME  (nColor_Sch)
            @ Yo+1,Xo+1 TO Yo+1,Xo+Ancho-2 COLOR SCHEME  (nColor_Sch)
            @ Yo+1,Xo+Ancho-1 SAY "´" COLOR SCHEME  (nColor_Sch)
      ENDCASE
   ENDIF
      ...
      *
      IF Listar
         IF Set_Pinta
            ...
            @ Yo+1,Xo+1 FILL TO Y1,X1-1 COLOR SCHEME (nColor_Sch)
         ENDIF
         LinTmp = LinIni
         DO WHILE (LinTmp <= LinFin) .AND. (&RegVal) .AND. (.NOT.EOF())
            ...
            ENDIF
            IF Set_Pinta
               @ LinTmp,Xo+2 SAY Contenido COLOR SCHEME  (nColor_Sch)
            ENDIF
            LinTmp                = LinTmp + 1
      ...
      * ---------------------------------------------------------------
      ...
      IF Set_Append .AND. ! Consulta
         ...
         * Edita la l¡nea activa.
         ...
         IF UltTecla <> escape_
            ...
            NumRg(Actual)  = RECNO()
            @ LinAct,Xo+2 SAY Linea(Actual) COLOR SCHEME (nColor_Sch)
      ...
      ELSE
         ...
         *
         LinAct = LinIni+Actual-1
         IF Ultimo <> 0
            @ LinAct,Xo+2 SAY Linea(Actual) COLOR SCHEME (nColor_Sch2)
            IF HAYSELLIN
               GOTO NumRg(Actual)
               DO &SelLin
            ENDIF
...
*       º                                                         º
*       º Author's Name   Victor E. Torres Tejada                   º
*       º                                                         º
*       º Copyright (c) 1993 Company Name o-Negocios                º
*       º Address                                                 º
*       º City,     Zip                                           º
*       º                                                         º
*       º Description:                                            º
...
function Elige
**************
PARAMETERS OpcDef,Yo,Xo,LenVec,PlMostrar
...
IF PlMostrar
	@ Yo-Yo2,Xo GET OpcAct FUNCTION &LsFunction.  color scheme 12
	CLEAR GETS 
ELSE
	@ Yo-Yo2,Xo GET OpcAct FUNCTION &LsFunction.  color scheme 16
	READ CYCLE MODAL
	UltTecla = LASTKEY()
ENDIF
...
******************************************************************************
procedure lib_merr
PARAMETERS nMensaje,lNoMostrar
IF PARAMETERS()<2
	lNoMostrar = .F.
ENDIF
...
@ Yo,Xo FILL  TO (Yo+Largo-1),(Xo+Ancho-1) COLOR SCHEME 18
@ Yo,Xo TO (Yo+Largo-1),(Xo+Ancho-1) COLOR SCHEME 18
   ...
   *ENDIF
   ...
   DO WHILE Ciclo
      IF Listar
         DO BPinta
      ENDIF
      @ Yo+1+LinIni,(Xo+Ancho-1) SAY IIF(Inicio ,"þ","") COLOR SCHEME 18
      @ Yo+Largo-2 ,(Xo+Ancho-1) SAY IIF(Fin    ,"þ","") COLOR SCHEME 18
      LinAct = Yo + Actual + LinIni
      IF Ultimo <> 0
         Contenido = Linea1(Actual)
         @ LinAct,Xo+1 SAY Contenido SIZE 1,Ancho-2 COLOR SCHEME 17
         IF ! Program==[]
            GOTO NumRg1(Actual)
            DO (Program)
         ENDIF
...
@ Yo+1,Xo+1 CLEAR TO (Yo+Largo-2),(Xo+Ancho-2)
@ Yo+1,Xo+1 FILL  TO (Yo+Largo-2),(Xo+Ancho-2) COLOR SCHEME  18
IF ! TRIM(Titulo)==""
   @ Yo+1,Xo+1 SAY Titulo SIZE 1,Ancho-2 COLOR SCHEME 18
   @ Yo+2,Xo+0 SAY "Ã" COLOR SCHEME 18
   @ Yo+2,Xo+1 TO Yo+2,Xo+Ancho-2 COLOR SCHEME 18
   @ Yo+2,Xo+Ancho-1 SAY "´" COLOR SCHEME 18
   LinIni = 2
   MaxLin = Largo - 4
ELSE
   LinIni = 0
...
DO WHILE (LinTmp < MaxLin) .AND. RegVal() .AND. (! EOF())
   ...
   NumRg1(LinTmp)  = RECNO()
   @ Yo+LinTmp+LinIni,Xo+1 SAY Linea1(LinTmp) SIZE 1,Ancho-2 COLOR SCHEME 18
   nSkip = 1
   DO Skip_Dn WITH nSkip
   IF nSkip > 0
      EXIT
...
LinTmp = LinTmp + 1
IF LinTmp  <= MaxLin
   @ Yo+LinTmp+LinIni,Xo+1 SAY "ð Fin "+REPLICATE("ð",Ancho-8) COLOR SCHEME 18
ENDIF
Listar = .F.
IF Actual > Ultimo
   Actual = Ultimo
...
PROCEDURE BDown
****************
@ Yo+Actual+LinIni,Xo+1 SAY Linea1(Actual) SIZE 1,Ancho-2 COLOR SCHEME 18
IF Actual=Ultimo
   GOTO NumRg1(Actual)
   nSkip = 1
   DO Skip_Dn WITH nSkip
...
DO WHILE nSkip > 0 .AND. ! BOF()
   @ Yo+Largo-2,(Xo+Ancho-1) SAY SUBSTR("\³/-",j,1) COLOR SCHEME 18
   J = IIF(J=4,1,J+1)
   SKIP -1
   IF INKEY() = 32
      RETURN
...
DO WHILE nSkip > 0 .AND. ! EOF()
   @ Yo+1+LinIni,(Xo+Ancho-1) SAY SUBSTR("\³/-",j,1) COLOR SCHEME 18
   J = IIF(J=4,1,J+1)
   SKIP +1
   IF INKEY() = 32
      ?? CHR(7)
...
IF _WINDOWS OR _MAC
	...
	@ WROWS()-5,0
	if nMensaje <> 0
	   @ WROWS()-5,40-LEN(cDesMsg)/2 SAY cDesMsg COLOR W*/N
	ENDIF
...
function fondo
**************
PARAMETER cTit1,cTit2,cTit3,cTit4,xscheme
IF PARAMETERS()<2
	STORE "" TO cTit2,cTit3,cTit4
ENDIF
IF PARAMETERS()<5
	xScheme = 16
ENDIF
...
RETURN .F.
**********************
FUNCTION verifyVar_Pln
**********************
PARAMETERS _cVar,_cType
#include CONST.H
IF VARTYPE(&_cVar)<>UPPER(_cType)
	=MESSAGEBOX('Debe definir en la opcion de -Configuraciones Generales- en el menu de configuración del modulo de Planilla'+CRLF + ;
	'la variable '+UPPER(_cVar)+' y asociarle los items correspondientes al valor de esta variable'+CRLF + ;
	'en el detalle de tablas generales de planilla asignando los codigos de conceptos'+CRLF + ;
	'de calculo y/o resultado que correspondan.'+CRLF + ;
	'Ejemplo: Tablas generales de planilla -> 51 - Netos a Pagar '+CRLF + ;
	'Y en detalle de tablas planilla, colocar los valores que correspondan,'+CRLF + ;
	'Adelantos de quincena -> [SC10] , Neto a Pagar -> [RM01] ',0+64,'Aviso importante !!!')
	RETURN .F.
ELSE
	RETURN .T.	
ENDIF
...
*********************************
FUNCTION Cheq_Resolucion_Pantalla
*********************************
PARAMETERS PnAncho,PnAlto  
***			Width ,Height
IF dimensiones()=.T.
...
ENDFUNC
****************************
FUNCTION VFP_Update_Cbdmauxi
****************************
PARAMETERS cClfAux,cCodAux,cNomAux,cTipoOperacion
LOCAL LlOk as Boolean
LlOk = -1
DO CASE 
	CASE cTipoOperacion='I'
		INSERT INTO CbdmAuxi (ClfAux,CodAux,NomAux,RucAux)  VALUES (cClfAux,cCodAux,cNomAux,cCodAux)
		LlOk = 1
	CASE cTipoOperacion='A'
		UPDATE CbdmAuxi	SET NomAux = cNomAux WHERE ClfAux+CodAux=cClfAux+cCodAux
		LlOk = 1
ENDCASE
RETURN LlOk