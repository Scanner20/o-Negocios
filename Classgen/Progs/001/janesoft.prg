...
if empty(lsWhere)
	...
	*
	DO CASE
	   ...
	   CASE UPPER(PcEntidad) = [CPIDO_TB]  &&& Detalle
        	SELECT Cpido_tb.*,Almcatge.DesMat,Almcatge.UndStk,Almtalma.DesSub,RECNO('Cpido_tb') AS NroReg, 0 AS RegGrb ;
        	FROM   Cpido_tb INNER JOIN Almtalma ON ;
        	       Cpido_tb.SubAlm = Almtalma.SubAlm ;
        	       INNER JOIN Almcatge ON ;
		           Cpido_tb.CodMat = Almcatge.CodMat ;
		    WHERE  &LsWhere. ;
		    INTO CURSOR Temporal
	        
	   CASE UPPER(PcEntidad) = [CPIMO_TB]  &&& Mano de Obra
        	SELECT Cpimo_tb.*, Plnmpers.ApePers+[, ]+Plnmpers.NomPers AS NomPers, RECNO('Cpimo_tb') AS NROREG ;
        	FROM   Cpimo_tb INNER JOIN Plnmpers ON ;
        	       Cpimo_tb.CodPers = Plnmpers.CodPers ;
		    WHERE  &LsWhere. ;
		    INTO CURSOR Temporal
	   
	   CASE UPPER(PcEntidad) = [CPIQO_TB]  &&& Maquinaria y Equipo	   	   
        	SELECT Cpiqo_tb.*, Cbdmpart.Nompar, RECNO('Cpiqo_tb') AS NROREG ;
        	FROM   Cpiqo_tb INNER JOIN Cbdmpart ON ;
        	       Cpiqo_tb.CodPar = Cbdmpart.CodPar ;
		    WHERE  &LsWhere. ;
		    INTO CURSOR Temporal
	   
	   OTHERWISE
  	        SELECT * FROM (PcEntidad) WHERE &LsWhere. INTO CURSOR Temporal
	ENDCASE
	*
	SELE Temporal
	LcArcTmp = GoEntorno.TmpPath+SYS(3)
...
IF GoCfgCpi.Crear
   ...
   *jj*     RETURN
   *jj* ENDIF
   *jj* IF sNroO_T = m.NroO_T
         * correlativo automatico
   *jj*    XsNroMes=TRAN(_MES,"@L ##")
   *jj*    Campo1   = [NDOC]+XsNroMes
   *jj*    LnNroO_T = CDOC.&Campo1.
   *jj*    LnNroO_T = VAL(XsNroMes+RIGHT(TRANSF(LnNroO_T,"@L ###"),3))
   *jj*    LsNroO_T = ALLTRIM(STR(LnNroO_T))
   *jj*    sNroO_T  = LEFT(CDOC.Siglas,3)+PADL(LsNroO_T,LEN(CO_T.NroDoc)-3,'0')
   *jj*    REPLACE &Campo1. WITH &Campo1. + 1
   *jj* ELSE
   *jj*    SELE CO_T
   *jj*    SEEK sNroO_T
   *jj*    IF FOUND()
   *jj*       GO iRECNO
   *jj*       DELETE
   *jj*       sErr = [Registro Creado por Otro Usuario]
   *jj*       DO F1MSGERR WITH sERR
   *jj*       RETURN
   *jj*    ELSE
   *jj*       SELE CDOC
   *jj*       sNroCorr = SUBSTR(sNroO_T,4)
   *jj*       IF VAL(sNroCorr)>=NroDoc
   *jj*          REPLACE NroDoc WITH VAL(sNroCorr)+1
   *jj*       ENDIF
   *jj*    ENDIF
   *jj*  ENDIF
   *jj*  UNLOCK IN "CDOC"
   *jj* SELE CO_T
   *jj* GO iRECNO
   *jj* REPLACE NroDoc WITH sNroO_T
   *
   SELECT CO_T
...
IF m.Insumos
   ...
   DO WHILE !EOF()
      ...
      LsDesCrip = LsDesCrip +[ EN:]+LEFT(GoCfgCpi.AlmNombr(LsSubAlm),13)  
      WAIT WINDOW [ACTUALIZANDO ]+LsDescrip NOWAIT
      
      GoCfgAlm.sNroDoc=ChkNroDoc(GoCfgCpi.sNroO_T+GoCfgAlm.SubAlm+GoCfgAlm.cTipMov+GoCfgAlm.sCodMov)
      IF !Grabar_transaccion_Alm(GoCfgAlm.SubAlm,GoCfgAlm.cTipMov,GoCfgAlm.sCodMov,GoCfgAlm.sNroDoc)
         =MESSAGEBOX([IMPOSIBLE GENERAR ]+GoCfgCpi.aDesMov(m.NumEle)+[ EN ]+GoCfgCpi.ALMNOMBR(LsSubALm))
         SELE C_DO_T
         SKIP
         LOOP
      ENDIF
      LsDescrip = LsDescrip + [Guía:]+LsNroDoc
      WAIT WINDOW [ACTUALIZANDO ]+LsDescrip NOWAIT
      LnNroItm = 0
...
ELSE
   ...
   SCAN WHILE NroDoc = GoCfgCpi.sNroO_T FOR EVAL(LsEvalua)
        ...
        WAIT WINDOW [ACTUALIZANDO ]+LsDescrip NOWAIT
        =chqmovalm_Pt()
        LsNroDoc=ChkNroDoc(GoCfgCpi.sNroO_T+LsSubAlm+LcTipMov+LsCodMov)
...
ELSE 
	GoCfgAlm.Crear  = .t.   
	m.NroDocAlm=correlativo_alm(GoCfgAlm.EntidadCorrelativo,m.cTipMov,m.sCodMov,m.cSubAlm,'0')
ENDIF
...
=Grabar_transaccion_Alm(m.SubAlm,m.TipMov,m.CodMov,m.NroDoc)
...
LsNroO_T  = PADR(GoCfgCpi.sNroO_T,LEN(DTRA.NroRef))
SELE DTRA
SET ORDER TO DTRA04
...
sele dtra
set order to dtra04
seek ZsTpoRef + GoCfgCpi.snroO_t
if found()
   scan while tporef=ZsTpoRef and nroref=GoCfgCpi.snroo_t and !eof()
        lpro_term=.f.
        for ii=1 to ntotmov
