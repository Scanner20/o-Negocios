...
Define Class ExcelXml As Custom
    ...
    * Arreglo para los titulos o leyendas antes del encabezado de las columnas / 	
 	DIMENSION aTitles[1,1]
 	CodStyleTit =''
	...
	*| ExcelXml::
	Procedure AddNewStyle
		Lparameters plcType, plnRow, plnCol, ;
			...
			plcFormat,plcWrapText,plcTagHeader
	
		IF VARTYPE(plcWrapText)<>'C'
			plcWrapText =''
		ENDIF

		IF VARTYPE(plcTagHeader)<>'C'
			plcTagHeader = ''
		ENDIF
		
		** VETT  19/02/18 09:53 : Respetar el ancho del campo aun si esta vacío
		plcWrapText		= PADR(plcWrapText,LEN(xxxStylesProperties.sswraptext))   
		plcTagHeader	= PADR(plcTagHeader,LEN(xxxStylesProperties.sstagheader))
		** VETT  19/02/18 09:53 :  
		...
		*- Adiciono no cursor caso nï¿½o ache um registro com os mesmos dados
		If Not Seek(plcAlignH + plcAlignV + plcFontName + plcFontFamily + ;
						plcFontSize + plcForeColor + plcFontBold + plcFontItalic + ;
						plcFontUnderline + plcFontStrikeThru + plcBackColor + plcPattern + ;
						plcFormat + lcTop + lcBottom + plcWrapText + plcTagHeader ,;
			...
			*- xml de estilo da celula
			lcXmlStyle = [  <Style ss:ID="] + lcStyleCode + [">] + This.crlf + ;
				[   <Alignment ss:Horizontal="]+alltrim(plcAlignH)+[" ss:Vertical="]+alltrim(plcAlignV)+IIF(!EMPTY(plcWrapText),[" ss:WrapText="]+ALLTRIM(plcWrapText),"") +["/>] + this.CRLF +;
		...
		** VETT  14/02/2018 03:47 PM : Guardamos el estilo para los titulos  	
		IF xxxStylesProperties.ssCode='t' 
			This.CodStyleTit = xxxStylesProperties.ssCode
		ENDIF
		Return lcXmlStyle
	Endproc
	...
	*| ExcelXml::
	Procedure BuildColumnsStyles
		...
		Local lcAlignH, lcAlignV, lcFontName, lcFontFamily, ;
			...
			lcFormat, lcXmlBorderStyle, lcXmlStyles, lnRow, lnCol, lnRowFound,lcWrapText,lcTagHeader
		...
		*- Verifico os estilos de todas as linhas/colunas do grid
		...
		Scan
			...
			For lnCol = 1 To This.GridObject.ColumnCount
				...
				If Not loColumn.Visible 	&&-Considero somente as colunas visiveis
					Loop
				Endif
				LcWrapText = "" && iif(loColumnHeader.WordWrap, "1", "")
				LcTagHeader= "BTRL" && ALLTRIM(loColumnHeader.Tag)
				...
				*- se o estilo jï¿½ existir "lcXmlStyle" retorna ""
				...
				lcXmlStyle = This.AddNewStyle( "c", lnRow, lnCol, ;
							...
							lcFontSize, lcForeColor, lcFontBold, lcFontItalic, ;
							lcFontUnderline, lcFontStrikeThru, lcBackColor, lcPattern, ;
							lcFormat,LcWrapText,LcTagHeader )
		...
		Return lcXmlStyles
	Endproc
	** VETT  14/02/2018 03:05 PM : Construimos el estilo para los titulos del reporte en caso existan
	
	PROCEDURE BuildTitlesStyles 
		...
		Local loColumn, loColumnHeader, lnCol, lcXmlStyles, lcXmlStyle, ;
			...
			lcFontFamily, lcPattern, lcAlignH, lcAlignV, lcCollate,LcWrapText,LcTagHeader
	
		lcXmlStyle = ""
		lcXmlStyles = ""
		LnColStyTit=1
		IF VARTYPE(This.GridObject.columnstyletitles)='N'
			LnColStyTit= IIF(This.GridObject.columnstyletitles>0,This.GridObject.columnstyletitles,1)
		ENDIF
		LnCol = IIF(This.GridObject.ColumnCount<LnColStyTit,1,LnColStyTit) 

		loColumn = This.GetColumn(LnCol)
		loColumnHeader = This.GetColumnHeader(loColumn)

		lcBackColor = 	This.ColorToStrHexa(Rgb(255, 255, 255)) && This.ColorToStrHexa( Iif(This.SetStyles, loColumnHeader.BackColor, Rgb(255, 255, 255)) )
		
		lcForeColor = 	This.ColorToStrHexa(Rgb(0, 0, 0)) && This.ColorToStrHexa( Iif(This.SetStyles, loColumnHeader.ForeColor, Rgb(0, 0, 0)) )

		
		lcFontName = Padr(loColumnHeader.FontName, Len(xxxStylesProperties.ssFontName))
		lcFontSize = Transform(loColumnHeader.FontSize+2, "@L 999")				
		...
		LcWrapText = iif(loColumnHeader.WordWrap, "1", "")
		LcTagHeader= ALLTRIM(loColumnHeader.Tag)
	
		*- se o estilo jï¿½ existir "lcXmlStyle" retorna ""
		lcXmlStyle = This.AddNewStyle( "t", 1, lnCol, ;
					lcAlignH, lcAlignV, lcFontName, lcFontFamily, ;
					lcFontSize, lcForeColor, lcFontBold, lcFontItalic, ;
					lcFontUnderline, lcFontStrikeThru, lcBackColor, lcPattern, ;
					lcFormat,LcWrapText,LcTagHeader )
	...
	Procedure BuildHeadersStyles
		...
		*- Crio cursor para armazenar todos os estilos encontrados
		...
		Index On ssAlignH + ssAlignV + ssFontName +	;
			...
			ssBackColor + ssPattern + ssFormat	+ ssTop + ssBottom + ssWrapText + ssTagHeader  Tag idxStyle
	...
	*| ExcelXml::
	Procedure BuildRows
		...
		*- Adicionamos filas de los titulos o leyendas superiores
		*!*	Por ahora usamos el estilo del encabezado(header) de la propiedad columnstyletitles del Grid 
		lcToolTipText = ""
	...
	*| ExcelXml::
	Procedure Save(plcFile)
		...
		*- por vï¿½rias cï¿½luas ou por uma ï¿½nica cï¿½lula.
		...
		lcStringStyles = lcStringStyles + This.BuildTitlesStyles()	&&- Estilo de los titulos del reporte
		lcStringStyles2 = lcStringStyles2 + This.buildcolumnsstyles()	&&- Estilos das linhas/colunas
		...
		** Verificamos si agregamos filas (rows) para titulos (Titles) o leyendas (legends) antes  de los encabezados (headers) de las columnas
		...
		Text To lcXmlWorksheet_part1 Textmerge Pretext 2 Noshow
			 <Worksheet ss:Name="<<This.SheetName>>">
			  <Table ss:ExpandedColumnCount="<<This.ColumnCount>>" ss:ExpandedRowCount="<<This.RowCount + LnTotTitles>>" x:FullColumns="1"
	...
	* 	56 = xlExcel8 (97-2003 format in Excel 2007-2013, xls)
	Procedure ConvertXmlToXlsx(tcFilename, tnFileFormat, tlOpenAfterExporting)
		...
		If 	VARTYPE(loExcel)<>'O'		&& !IsObject(loExcel)
