...
@ 0,82   SAY [ANO:]+TRAN(_ANO,[9999])      COLOR SCHEME 7
@ 0,54   SAY " Fecha : " + Dtoc(date())
IF XsCodPln=[1]
...
@ 15,4   SAY "Localidad  :"
@ 15,41  SAY "Ubic.Geog.:"
@ 16,4   SAY "Fch Nacimie:"
@ 16,31  SAY "Edad:"
@ 16,41  SAY "Provincia :"
@ 17,4   SAY "Nro. Hijos :"
@ 17,25  SAY "Carga Fami:"
@ 17,41  SAY "Est.Civil :"
@ 18,4   SAY "Carnet IPSS:"
@ 18,41  SAY "Carnet AFP:"
@ 19,4   SAY "Lib.Elect  :"
@ 19,41  SAY "Lib.Milit :"
@ 20,4   SAY "Cod. A.F.P.:"
@ 21,4   SAY "Fch Ingreso:"
@ 21,31  SAY "T.Serv.:    Años    Meses    Dias"
@ 22,4   SAY "Fecha Cese :"
@ 20,41  SAY "Fch AFP:"
@ 20,59  SAY "Fch.Con:"
@ 21,66  SAY "Pln :"
...
@ 22,55  SAY "Per. Vac.:"
*!*	DO LIB_MTEC WITH 3
DO f1_edit WITH 'PERLlave','PERMuest','PEREdita','PERElimi','PERlista',[],[],'CMAR',[]
RETURN
**********************************************************************
...
ENDIF
@ 16,54 SAY ProVin
@ 17,20 SAY NhiJos PICT "##"
@ 17,37 SAY NcgFam PICT "#"
@ 17,54 SAY EstCvl
DO CASE
  CASE EstCvl="1"
     @ 17,54 SAY "Soltero/a   "
  ...
  CASE EstCvl="3"
     @ 17,54 SAY "Viudo/a     "
  CASE EstCvl="4"
     @ 17,54 SAY "Divorciado/a"
  CASE EstCvl="5"
     @ 17,54 SAY "Conviviente "
  OTHER
     @ 17,54 say "            "
ENDCASE
@ 18,20 SAY N_ipss
...
@ 19,20 SAY LElect
@ 19,54 SAY LMilit
@ 20,20 SAY CodAfp  pict "99"
  =SEEK('23'+PERS.CodAfp,'TABL')
@ 20,23 SAY LEFT(TABL.Nombre,12)
@ 21,20 SAY FchIng
IF CTOD("  /  /  ") <> FchIng
...
DO WHILE ! INLIST(UltTecla,Escape,CtrlW,F10)
   DO CASE
      CASE i = 1
         ...
         SAVE SCREEN
         @ 2,47 TO 6,89
         @ 3,49 SAY "APELLIDO PATERNO :" GET M.PATERNO PICT "@!"
         @ 4,49 SAY "APELLIDO MATERNO :" GET M.MATERNO PICT "@!"
         @ 5,49 SAY "NOMBRES          :" GET M.NOMBRES PICT "@!"
         READ
         RESTORE SCREEN
         ULtTECLA = LASTKEY()
         XNOMPER = M.PATERNO+M.MATERNO+M.NOMBRES
         @  3,49 SAY SPACE(36)
         @  3,49 SAY TRIM(LEFT(XNomPer,20))+" "+TRIM(SUBSTR(XNOMPER,21,20))+" "+TRIM(RIGHT(XNOMPER,20)) PICT "@S36"
      CASE i = 2
         SELE TABL
         XsTabla  = '01'
      ...
      CASE i = 4
         ...
         IF !EMPTY(XGra_Sg)
            ...
            @  4,71 SAY LEFT(Tabl.NomBre,14) PICT "@!"
         ENDIF
      CASE I = 5
         XsTabla = "29"
      ...
      CASE i = 10
         SELECT TABL
         XsTabla = "07"
         @  7,70 GET XGrdIns PICT REPLICATE("9",Len(XGrdIns))
         READ
         ULTTECLA=Lastkey()
         IF UltTecla == Escape
            EXIT
         ...
         @  7,73 SAY Left(TRIM(TabL->Nombre),8)
      CASE i = 11
         SELE TABL
         XsTabla  = '05'
      ...
      CASE i = 12
         SELECT TABL
         XsTabla  = '08'
         @  8,70 GET XProPer PICT REPLICATE("9",Len(XProPer))
         READ
         ULTTECLA=Lastkey()
         IF UltTecla = Escape
            EXIT
      ...
      CASE i = 15
         @  9,70 GET XNroTar PICT "##########"
         READ
         ULTTECLA=Lastkey()
         IF UltTecla = Escape
            EXIT
         ENDIF
         @  9,70 SAY XNroTar PICT "##########"
      CASE i = 16
         SELE TABL
         XsTabla  = 'GP'
         @ 11,18 GET XGrpPer PICT REPLICATE("9",Len(XGrpPer))
         READ
         UltTecla= Lastkey()
         IF UltTecla = Escape
            EXIT
         ...
         @ 11,18 SAY XGrpPer
         IF ! FOUND()
            GsMsgErr = "Código Lugar de Pago no Registrado"
         ENDIF
      CASE I =17
         SELE TABL
         XsTabla  = 'CT'
         @ 11,39 GET XTpoCta PICT REPLICATE("9",Len(XTpoCta))
         READ
         UltTecla= Lastkey()
         IF UltTecla = Escape
            EXIT
         ...
         ENDIF
      CASE i = 18
         @ 12,42 GET XCtaHor PICT "@!"
         READ
         UltTecla= Lastkey()
         @ 12,42 SAY XCtaHor
      CASE i = 19
         XsTabla = "02"
         @ 11,70 GET XCtaCts PICT REPLICATE("9",LEN(XCtaCts))
         READ
         ULtTECLA = LASTKEY()
         IF UltTecla = F8
      ...
      CASE i = 20
         ...
         @ 14,20 SAY XDirecc
      CASE i = 21
         @ 14,60 GET XTlfPer PICT REPLICATE("9",Len(XTlfPer))
         READ
         ULTTECLA=Lastkey()
         IF UltTecla = Escape
            EXIT
      ...
      CASE i = 23
         SELECT TABL
         XsTabla  = '11'
         @ 15,54 GET XUbiGeo PICT REPLICATE("9",Len(XUbiGeo))
         READ
         ULTTECLA=Lastkey()
         IF UltTecla = Escape
            EXIT
      ...
      CASE i = 24
         ...
         ENDIF
      CASE i = 25
         @ 16,54 GET XProvin PICT "@!"
         READ
         ULTTECLA = LASTKEY()
         @ 16,54 SAY XProvin
      CASE i = 26
         @ 17,20 GET XNhijos PICT "99"
         READ
         ULTTECLA = LASTKEY()
         @ 17,20 SAY XNhijos PICT "99"
      CASE i = 27
         @ 17,37 GET XNcgfaM  PICT "9"
         READ
         ULTTECLA = LASTKEY()
         @ 17,37 SAY XNcgFam  PICT "9"
      CASE i = 28
         VecOpc(1)="Soltero/a   "
         VecOpc(2)="Casado/a    "
         ...
         ENDCASE
      CASE i = 29
         @ 18,20 GET XN_ipss PICT "@!"
         READ
         UltTecla= Lastkey()
         @ 18,20 SAY XN_ipss PICT "@!"
      CASE i = 30
         @ 18,54 GET XCarAfp PICT "@!"
         READ
         UltTecla= Lastkey()
         @ 18,54 SAY XCarAfp PICT "@!"
      CASE i = 31
         @ 19,20 GET XLElect PICT "@!"
         READ
         UltTecla= Lastkey()
         @ 19,20 SAY XLElect
      CASE i = 32
         @ 19,54 GET XLMilit PICT "@!"
         READ
         UltTecla= Lastkey()
         @ 19,54 SAY XLMilit
      CASE i = 33
         SELECT TABL
         XsTabla  = '23'
         @ 20,20 GET XCodAfp PICT REPLICATE("9",Len(XCodAfp))
         READ
         ULTTECLA=Lastkey()
         IF UltTecla = Escape
            EXIT
         ...
         IF ! FOUND()
            GsMsgErr = "Código de Afp No Registrado"
         ENDIF
         @ 20,23 SAY Left(TabL->Nombre,12)
      CASE i = 34
         @ 21,20 GET XFchIng
