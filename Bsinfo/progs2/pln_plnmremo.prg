...
ENDIF
SELE PERS
SET FILTER TO CodPln = XsCodPln .AND. EMPTY(FchCES) AND FCHING<{01/01/85}
IF XsCodPln = "1"
   SELECT 2
...
PRINTJOB
  ...
  DO WHILE !EOF()
     ...
     @NUMLIN, 52 say _PAGENO
     @NUMLIN+1,0 say GSDIRCIA
     @NUMLIN+2,0 say "R.U.C.: "+GSRUCCIA

     @NUMLIN+4,0 SAY PADC('LIQUIDACION DE COMPENSACION POR TIEMPO DE SERVICIOS',80)
...
*     TOTAL                 :                          = 99,999.99
     ...
     @NUMLIN+1,2 SAY 'EN FE DE LO CUAL FIRMA EL PRESENTE DOCUMENTO'
      numlin=numlin+3
     if gscodcia="003"
     @NUMLIN, 2 SAY "Ca¤ete, 15 Mayo de 1,997"
     else
     @NUMLIN, 2 SAY "Lima, 15 Mayo de 1,997"
     endif
