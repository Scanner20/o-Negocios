...
DIMENSION vTpoAst(40),vNroAst(40),vNotAst(40),vImport(40),VECOPC(5)
DIMENSION xCodCta(40),xCodMon(40),xNroRef(40),xCodAux(40),xCodOpe(40),xCodDiv(40),xNroPro(40),xFchPro(40)
...
DO CASE
	CASE gocfgcbd.C_COSTO = 3
		@ 4,30 SAY "C.COSTO:" COLOR SCHEME 11
		@ 5,30 SAY "S.COSTO:" COLOR SCHEME 11
		@ 6,30 SAY "PARTIDA:" COLOR SCHEME 11
	CASE gocfgcbd.C_COSTO = 2 
		@ 4,30 SAY "C.COSTO:" COLOR SCHEME 11
	CASE gocfgcbd.C_COSTO = 1	
		@ 4,30 SAY "C.COSTO:" COLOR SCHEME 11
	OTHER
		@ 4,30 SAY "AUXIL:  " COLOR SCHEME 11
ENDCASE
...
DO WHILE .NOT. INLIST(UltTecla,F10,CtrlW,Escape_)
   ...
   **    @ 04,33 SAY XSCODFIN+" "+SUBSTR(AUXI->NOMAUX,1,12)
		...
		CASE I = 15
			...
			ENDIF
		CASE I = 16
			@ 19, 14 GET XsGirado PICT "@!" FONT "Tahoma",9 STYLE 'B'
			READ
			UltTecla = LastKey()
			@ 19, 14 SAY PADR(XsGirado,70) PICT "@!" FONT "Tahoma",9 STYLE 'B'
		CASE I = 17
...
IF EMPTY(XsGirado) .OR. MaxEle = 1
	...
	ENDIF
	@ 19,14 SAY PADR(XsGirado,70) PICT "@S50" FONT "Tahoma",7 STYLE 'B'
ENDIF
SET ORDER TO VMOV01
RETURN .T.
...
******************************************************************************
* Objeto : Procedure Imprime Cheque
******************************************************************************
PROCEDURE MovPrin3_bws  && Cambio de Pantalla de Reporte para cheque MAAV
**********************
LOCAL lcRptTxt,lcRptGraph,lcRptDesc
LOCAL LcArcTmp,LcAlias,LnNumReg,LnNumCmp,LlPrimera
LcArcTmp=GoEntorno.TmpPath+Sys(3)
...
IF EOF()
	...
	RETURN
ENDIF
lcRptTxt   = "cja_cheque_bws"
lcRptGraph = "cja_cheque_bws"
lcRptDesc  = "Cheque Scotiabank"
LoTipRep   = ''
IF .F.
	MODI REPORT cja_cheque_bws
ENDIF
DO FORM ClaGen_Spool WITH lcRptTxt , lcRptGraph , '1' , 2 , lcRptDesc &&, THISFORM.DATASESSIONID
USE IN VMOV_T
USE IN RMOV_T
...
* Objeto : Procedure Imprime Cheque
******************************************************************************
PROCEDURE MovPrin3_bcp  && Cambio de Pantalla de Reporte para cheque MAAV
**********************
LOCAL lcRptTxt,lcRptGraph,lcRptDesc
LOCAL LcArcTmp,LcAlias,LnNumReg,LnNumCmp,LlPrimera
LcArcTmp=GoEntorno.TmpPath+Sys(3)
...
IF EOF()
	...
	RETURN
ENDIF
lcRptTxt   = "cja_cheque_bcp"
lcRptGraph = "cja_cheque_bcp"
lcRptDesc  = "Cheque BCP"
LoTipRep   = ''
IF .F.
	MODI REPORT cja_cheque_bcp
ENDIF
DO FORM ClaGen_Spool WITH lcRptTxt , lcRptGraph , '1' , 2 , lcRptDesc &&, THISFORM.DATASESSIONID
USE IN VMOV_T
USE IN RMOV_T
...
* Objeto : Procedure Imprime Cheque
******************************************************************************
PROCEDURE MovPrin3_bbva  && Cambio de Pantalla de Reporte para cheque MAAV (29-01-2007)
***********************
LOCAL lcRptTxt,lcRptGraph,lcRptDesc
LOCAL LcArcTmp,LcAlias,LnNumReg,LnNumCmp,LlPrimera
LcArcTmp=GoEntorno.TmpPath+Sys(3)
...
IF EOF()
	...
	RETURN
ENDIF
lcRptTxt   = "cja_cheque_bbva"
lcRptGraph = "cja_cheque_bbva"
lcRptDesc  = "Cheque BBVA"
LoTipRep   = ''
IF .F.
	MODI REPORT cja_cheque_bcp
ENDIF
...
IF EOF()
	...
	RETURN
ENDIF
lcRptTxt   = "cja_voucher"
lcRptGraph = "cja_voucher"
lcRptDesc  = "Cheque"
LoTipRep   = ''
IF .F.
	MODI REPORT cja_voucher
ENDIF
DO FORM ClaGen_Spool WITH lcRptTxt , lcRptGraph , '1' , 2 , lcRptDesc &&, THISFORM.DATASESSIONID
USE IN VMOV_T
USE IN RMOV_T
...
@  0,6  SAY _PRN7A+TRIM(GsNomCia)
@  0,0  SAY _PRN7B
@  0,78 SAY _PRN7A+"VOUCHER "+LEFT(VMOV->NroAst,2)+"-"+SUBSTR(VMOV->NroAst,3,2)+"/"+SUBSTR(VMOV->NroAst,6,4)
@  0,0  SAY _PRN7B
...
CREATE TABLE (LcArcRmov_t) FREE ( NroMes c(LEN(RMOV.NroMes)), CodOpe c(LEN(RMOV.CodOpe)),;
								...
								FchPro d(8),ImpDbe n(16,2),ImpHbe n(16,2) )
