...
* MODIFCADO  : VETT 2008/11/27 13:52:11 - HABILITADO PARA PROFORMAS         *
*****************************************************************************
...
LoDatAdm.abrirtabla('ABRIR','VTAVPROF','GDOC','GDOC01','')
...
** relaciones a usar **
SELECT TDOC
SET RELATION TO CodDoc INTO DOCS
SELECT RDOC
...
*1    Codigo : RECI  Numero : 1234567890                     Fecha : 99/99/99
*2 Cliente   : 1234567 12345678901234567890123456789012345 Emision : 99/99/99
...
*!*	XScheme = K
*!*	nColor_Sch = K
DO fondo WITH 'NOTAS DE CREDITO - PROFORMAS',Goentorno.user.login,GsNomCia,GsFecha,16
CLEAR
IF _windows OR _mac
	@  0,0 TO 22,100  PANEL
ELSE
...
@  2,2  SAY "Cliente   :                                             Emision :        "
@  3,2  SAY "Direccion :                                               RUC :          "
@  4,2  SAY "Detalle   :                                               Moneda :       "
@  5,2  SAY "                                                     Tpo. Cambio :       "
@  7,2  SAY "                                                                         "
@  7,2  SAY "        ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿"
@  8,2  SAY "        ³ Codigo       D e s c r i p c i o n                 Importe    ³"
@  9,2  SAY "        ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´"
@ 10,2  SAY "        ³                                                               ³"
@ 11,2  SAY "        ³                                                               ³"
@ 12,2  SAY "        ³                                                               ³"
@ 13,2  SAY "        ³                                                               ³"
@ 14,2  SAY "        ³                                                               ³"
@ 15,2  SAY "        ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´"
@ 16,2  SAY "        Ú Documento a Asignar ÄÄÄÄÄ¿      ³ TOTAL                       ³"
@ 17,2  SAY "        ³ C¢digo :                 ³      ³ SALDO ACTUAL                ³"
@ 18,2  SAY "        ³ N£mero :                 ³      ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ"
...
DO WHILE !INLIST(UltTecla,escape_)
   DO CASE
      CASE i = 1
         SELE TDOC
         SET FILTER TO TpoDoc=XsTpoDoc AND CodDoc=DOCS.CodDoc AND DOCS.TpoBiz='I'
         LOCATE
         ...
         * definimos # del documento *
         XsNroDoc = PADR(PADL(ALLTRIM(STR(TDOC->NroDoc)),LEN(GDOC->NroDoc),'0'),LEN(GDOC->NroDoc))
         m.NroDoc = XsNroDoc
         SET FILTER TO
...
@  1,14 SAY CodDoc
@  1,29 SAY NroDoc
@  1,68 SAY FchDoc pict '@d dd/mm/aa'
...
@  4,69 SAY IIF(CodMon=2,'US$','S./')
@  5,69 SAY TpoCmb PICT "9,999.9999"
...
@  5,69 GET XfTpoCmb PICT "9,999.9999"
@ 17,21 GET XsCodRef
@ 18,21 GET XsNroRef
CLEAR GETS
UltTecla = 0
...
DO WHILE !INLIST(UltTecla,escape_)
   ...
   DO CASE
      ...
      CASE i = 8
         ...
         VecOpc(1)="S/."
         VecOpc(2)="US$"
         XiCodMon= Elige(XiCodMon,4,69,2)
      CASE i = 9
         SELE TCMB
         SEEK DTOS(XdFchDoc)
         =SEEK(XsCodOpe,"OPER")
         XfTpoCmb = IIF(OPER.TpoCmb=1,OfiCmp,OfiVta)
         @  5,69 GET XfTpoCmb PICT "9,999.9999" RANGE 0,
         READ
         UltTecla = LASTKEY()
   ENDCASE
   IF i = 9 .AND. UltTecla = Enter
      EXIT
   ENDIF
   i = IIF(UltTecla=Arriba,i-1,i+1)
   i = IIF(i<1,1,i)
   ...
   DO CASE
      CASE i = 1
         ** OJO >> Se puede superar la asignacion **
         SELE TDOC
         SET FILTER TO TpoDoc = [CARGO] AND CodDoc='PROF'
...
*********************************
PROCEDURE BElimi_NC_Proforma_Asig
*********************************
...
** Posicionamos punteros,  ¡ Mismo micro !   
...
*!*	   REPLACE VMOV->Digita  WITH GsUsuario
   ...
   ** Utilizamos la moneda del documento de referencia **
   ...
   IF CTAS.PidDoc=[S]
      XsClfAux = CTAS.ClfAux
      XsCodDoc = IIF(SEEK(GsCodSed+VTOS.CodRef+'001','DOCM'),DOCM.TpoDocSN,'')
