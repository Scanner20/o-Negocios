...
Do FONDO WITH 'EGRESOS DE CAJA  '+cTitulo + '    USUARIO: '+ goEntorno.User.Login +'   '+' EMPRESA: '+TRIM(GsNomCia),'','',''
DIMENSION vTpoAst(80),vNroAst(80),vNotAst(80),vImport(80),VECOPC(5)
DIMENSION xCodCta(80),xCodMon(80),xNroRef(80),xCodAux(80),xCodOpe(80),xCodDiv(80),xNroPro(80),xFchPro(80), vCtaPr1(80)
DIMENSION vCodCta(80),vCodAux(80),vNroRef(80),vImpCta(80),vTpoMov(80),vCodDoc(80),vCodDiv(80),vCtaPre(80)

STORE '' TO vCodCta(80),vCodAux(80),vNroRef(80),vTpoMov(80),vCodDoc(80),vCodDiv(80),vCtaPre(80), vCtaPr1(80) 
STORE '' TO xCodCta(80),xNroRef(80),xCodAux(80),xCodOpe(80),xCodDiv(80),xNroPro(80)
STORE '' TO vTpoAst(80),vNroAst(80),vNotAst(80)
STORE 0  TO vImpCta(80),vImport(80),xCodMon(80)
STORE {} TO xFchPro(80)
	...
	*-----
	...
	DO CASE
		...
		OTHER
			...
			ENDIF
			IF (LnImprimir # 6 AND hasaccess('EgresosCaja_Modificar') ) OR (Crear AND hasaccess('EgresosCaja_Adicionar'))
			...
			IF UltTecla <> Escape_
				IF (LnImprimir # 6 AND hasaccess('EgresosCaja_Modificar') ) OR (Crear AND hasaccess('EgresosCaja_Adicionar'))
					DO MOVGraba
				ENDIF
				IF UltTecla <> Escape_ AND hasaccess('EgresosCaja_Consultar') 
...
LinGirado   = LinImpChq + IIF(_Dos,1,2) && 19
LinNota     = LinGirado + IIF(_Dos,1,2) && 20
LinProvee   = LinNota + IIF(_Dos,1,2) && 21
LinDetalles = LinProvee + IIF(_Dos,1,2)
...
@ YoBrow1,01 SAY PADR("ð Tipo  ð N§ PROV.ð         C O N C E P T O                ð         IMPORTE ð"+ PADR(UPPER(GoCfgCbd.GloPre),20)+"     ð",AnchoBrow1-2) COLOR SCHEME 7
...
@ YoBrow2,01 SAY "DV -CTAS.-   C O N C E P T O       ð AUXI.     ð TD.ðN§  DOCTO.ð          IMPORTE ð"+ PADR(UPPER(GoCfgCbd.GloPre),20)+"ð" COLOR SCHEME 7
*


@ LinImpChq,00 SAY "IMPORTE....:" COLOR SCHEME 11
@ LinGirado,00 SAY "GIRADO A...:" COLOR SCHEME 11
@ LinNota  ,00 SAY "CONCEPTO...:" COLOR SCHEME 11
@ LinProvee,00 SAY "PROVEEDOR..:" COLOR SCHEME 11
...
*!*							SET DEVICE TO SCREEN
	...
	ELSE
		...
		IF HasAccess('EgresosCaja_Consultar') OR HasAccess('EgresosCaja_Modificar')
			DO MovPinta
		ELSE
...
IF VARTYPE(VMOV.PARFIN)='C'
	=SEEK(PADR(VMOV.ParFin,LEN(PPRE.CtaPre)),"PPRE",'PPRE01')
...
@ LinProvee,14 SAY SPACE(LEN(RMOV->CodAux))
@ LinProvee,22 SAY SPACE(50)
	
Ancho    = AnchoBrow1
Xo       = INT((AnchoTotal - Ancho)/2)
	...
	DO CASE
		CASE EliItm = CHR(43)      
			...
			IF EMPTY(XsCodAux)
				XsCodAux = CodAux
				XsClfAux = ClfAux
				@ LinProvee,14 SAY XsCodAux
				=SEEK(XsClfAux+XsCodAux,"AUXI")
				XsNomAux = AUXI->NomAux
				@ LinProvee,26 SAY AUXI->NomAux PICT "@S60"
			ENDIF
			MaxEle1= MaxEle1+ 1
			*** Revisar ***
...
SELECT VMOV
	...
	@ LinImpChq ,18 SAY VMOV->ImpChq PICT "99,999,999,999.99"
	@ LinGirado ,14 SAY PADR(VMOV->Girado,70) FONT "Tahoma",7 STYLE 'B' 
	@ LinNota   ,14 SAY VMOV->NotAst
	...
	DO CASE
        ...
        *SELE VMOV
		...
		CASE I = 3
			...
			IF Crear
				XdFchPed = IIF(CfgFchFnzSegunAst,XdFchAst,{})
				XdFchEnt = IIF(CfgFchEntSegunAst,XdFchAst,{})
...
*!*				@ 03,10 GET cCodCta PICTURE REPLICATE("9",LEN(cCOdCta)) VALID _vlook_cja(@cCodCta,'CodCta','CTA10X','CTAS')
			@ 03,10 GET cCodCta PICTURE REPLICATE("9",LEN(cCOdCta)) VALID _vlook_cja(@cCodCta,'CodCta','CTAXXX','CTAS')
			READ
			UltTecla = LASTKEY()
*!*				IF UltTecla = Escape_
*!*					EXIT
*!*				ENDIF
   ...
   **    @ 04,33 SAY XSCODFIN+" "+SUBSTR(AUXI->NOMAUX,1,12)
		...
		CASE I = 19
			SAVE SCREEN
			@ LinDetalles,00 TO LinDetalles + 5 ,79 DOUBLE
			@ LinDetalles,03 SAY "DETALLES" COLOR SCHEME 7
			GsMsgKey = "[S-TAB] Campo anterior     [Esc] Cancela    [F10] Siguiente Campo "
			DO LIB_MTEC WITH 99
			@ LinDetalles + 1,1 EDIT XsGloAst SIZE 3,78 COLOR SCHEME 7
			READ
			RESTORE SCREEN
			UltTecla = LastKey()
	...
	IF INLIST(UltTecla,F10,CtrlW)
		IF XfImpChq < 0
			...
			i = 8
		ENDIF
		IF VARTYPE(CfgDocPorChqVou)='N' AND CfgDocPorChqVou>0 AND XsCodDo1='CH' 
			LnTot1 = 0
			FOR ii = 1 TO ALEN(vTpoAst)
				LnTot1 = LnTot1 + IIF(EMPTY(vTpoAst(ii)),0,1)
			ENDFOR
			LnTot2 = 0
			FOR zz = 1 TO ALEN(vCodCta)
				LnTot2 = LnTot2 + IIF(EMPTY(vCodCta(zz)),0,1)
			ENDFOR

			IF LnTot1 + LnTot2 > CfgDocPorChqVou
				LnRpt=MESSAGEBOX('El número de documentos ingresados excede el maximo soportado que es de '+ TRANSFORM(CfgDocPorChqVou,'999') +' para el formato de cheque voucher.       ¿ Desea continuar ?',4+32,'¡¡ ATENCION !! / ¡¡ WARNING !!')
				IF LnRpt=7
					UltTecla = 0
					i = 8
				ENDIF
		...
		ELSE
			IF XsCodDo1='CH'
				=MESSAGEBOX('No esta confgurado el numero de lineas que soporta el formato de cheque voucher la impresion del documento podria tener problemas si son mas de lo que soporta dicho formato.',64,' !!ATENCION ¡¡ / ¡¡ WARNING !! ')
			ENDIF	
		ENDIF
	ENDIF
ENDDO
...
DO ABROWSE
MaxEle1 = MaxEle
@ YoBrow1 + RowsBrow1 - 1,1   TO YoBrow1 + RowsBrow1 - 1,AnchoBrow1  - 2
...
=SEEK(vCtaPr1(NumEle),"PPRE",'PPRE01')
...
@ NumLin,84 SAY TRIM(vCtaPr1(NumEle)) + ' '+ LEFT(PPRE.Nombre,12)
RETURN
		...
		CASE i = 2
			...
			IF ULTTECLA = F8
				...
				*!*	LfSalAct2 = 0.00
				...
				LsNroAst_R = SPACE(10)
				ZiOpcion = 3
				DO FORM cja_cjaccpag2 TO LsNroAst_R WITH '',PROV.CodCta,LsTpoAst
				IF AT('@',LsNroAst_R)>0
				...
				MaxEle = MaxEle_Old
				NumEle = NumEle_Old
				DIMENSION vCodCta(80)
				=ACOPY(vCodcta_Old,vCodCta)
				=SEEK(LsTpoAst,'PROV','PROV01')
			...
			*
            =SEEK(LsCtaPr1,"PPRE",'PPRE01')			
			@ NumLin,84 SAY TRIM(LsCtaPr1)+' '+ LEFT(PPRE.Nombre,12)			
			*
		CASE i = 3
			@ NumLin,58 SAY IIF(XnCodMon=1,"S/.","US$")
...
vCtaPr1(i) = SPACE(LEN(RMOV->CtaPre))
Estado = .T.
RETURN
...
=SEEK(vCtaPre(NumEle),"PPRE",'PPRE01')
	...
	DO CASE
        ...
        *ENDIF
		...
		CASE i = 3
			IF CTAS->PIDAUX="S"
				...
				IF EMPTY(XsCodAux)
					...
					XsClfAux = XsClfAux
					@ LinProvee,14 SAY XsCodAux
					@ LinProvee,26 SAY AUXI->NomAux PICT "@S70"
				ENDIF
				IF EMPTY(XsGirado) &&.OR. MaxEle = 1
					IF LsCodAux = "999"
						XsGirado = PADR(AUXI->NomAux,LEN(XsGirado))
		...
		CASE i = 8        && C¢digo de Cuenta
			SELECT PPRE
			IF GoCfgCbd.TIPO_CONSO = 2
				SET FILTER TO CodDiv = TRIM(LsCodDiv)
			ENDIF
...
vTpoMov(i) = "D"
vImpCta(i) = 0
vCtaPre(i) = IIF(gocfgcbd.PidPre,XsParFin,SPACE(LEN(RMOV.CtaPre)))
Estado = .T.
RETURN
...
FOR NumEle = 1 TO Maxele2
	LlCtas=SEEK(vCodCta(NumEle),"CTAS")
...
*!*		IF Import = 0 .AND. EliItm = "*"   
	...
	@ LinAct,0  SAY _PRN4
	IF _Fontsize>8
		@ LinAct,07 SAY LsGloDoc    PICT "@S50" font _Fontname,8
	ELSE
		@ LinAct,07 SAY LsGloDoc    PICT "@S50" font _Fontname,_Fontsize -1
	ENDIF

	@ LinAct,0  SAY _PRN2
	@ LinAct,39+3+5 SAY CodCta
	@ LinAct,45+3+5 SAY NroDoc
	@ LinAct,55+3+5 SAY NroRef
	IF TpoMov = "D"
...
CREATE TABLE (LcArcVmov_t) FREE ( NroMes c(LEN(VMOV.NroMes)), CodOpe c(LEN(VMOV.CodOpe)),;
								...
								Auxil C(11), ParFin C(15), CodMon N(1) )
...
SCAN WHILE NroMes+CodOpe+NroAst = XsNroMes+XsCodOpe+XsNroAst
	...
	IF !FOUND()
		...
		REPLACE Auxil  WITH VMOV.Auxil
		REPLACE CodMon WITH VMOV.CodMon
		REPLACE ParFin WITH IIF(VARTYPE(VMOV.ParFin)='C',VMOV.ParFin,'')
	ENDIF
	SELECT RMOV
	SEEK XsNroMes+XsCodOpe+XsNroAst
	SCAN WHILE NroMes+CodOpe+NroAst=XsNroMes+XsCodOpe+XsNroAst
