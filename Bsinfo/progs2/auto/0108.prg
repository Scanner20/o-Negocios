&& Programa para paola, generar las cuentas desino d la 67 d Quimica, sistema antiguo
...
n = 0
sie = 0
SET DEFAULT TO H:\o-Negocios\Aromas\data\CIA004\c2010
...
reg = RECNO()
?RECNO() 
	IF nromes='08' and codope='010' and rmctas.codcta ='67601' THEN 
	ritm = rmctas.nroitm
	kitm = rmctas.chkitm
	SKIP		
		IF rmctas.codcta<>'97000'
			sk = 0 
			...
			*!*				replace chkitm WITH chkitm  + 2
			...
			n = n + 1 	 
			APPEND BLANK 
			replace nromes with mes,codope with ope ,nroast with ast,nroitm WITH ritm + 1,chkitm WITH kitm + 1 ,codcta WITH '97000'
			replace fchdoc WITH fch,tpocmb with cmb, tpomov WITH 'D', import WITH imp,codmon WITH mon
			replace impusa WITH usa, fchast WITH fch,coddiv WITH '01', Tipdoc WITH 'NO', codcco WITH '0005'
			replace eliitm WITH '*'
			APPEND BLANK 
