...
m.tipmat = 1
m.general = .F.  && .F. Solo el actual almacen .T. Todos los almacenes
ulttecla  = 0
m.control = 1
cancelar  = .F.
m.Sel_Col = .F.
...
IF !FILE(xsarcmdlo)
	...
	IF m.tipo=1
		CREATE TABLE &arch. free (codmat C(LEN(catg.codmat)), stkact N(14,3),ci00 N(14,4), ;
			...
			nivel C(3),stkini N(14,4),valini N(14,4),desmat c(LEN(catg.Desmat)) )
	ELSE
...
SCAN WHILE EVAL(lswhile)	
	...
	*ENDIF
	...
	regactual=RECNO()
	=SEEK(m.codmat,[CATG])
	LnLenDes1 = LEN(CATG.Desmat) - LEN(catg.CodAnt) - LEN(catg.undstk) - 2
	LnLenDes2 = LEN(CATG.Desmat) - LEN(catg.CodAnt) - 1
	LnLenDes3 = LEN(CATG.Desmat) - LEN(catg.UndStk) - 1
	LsCadena = LEFT(catg.desmat,LnLenDes1)
	Lscadena = PADR(RTRIM(LsCadena)+[ ]+ CATG.CodAnt,LnLenDes3)+[ ]+catg.undstk
	GO regactual
	...
	**REPLACE PrmIni WITH fPrmIni
	...
	DO WHILE EVAL(lswhile2) .AND. (dtra.fchdoc <= dfchdoch) .AND. ! EOF()
		IF dtra->subalm = "WF " && No altera los costos ***
			...
			LOOP
		ENDIF
		IF EMPTY(TipMov) OR EMPTY(CodMov)
			SELE dtra
			SKIP
			LOOP
		ENDIF
