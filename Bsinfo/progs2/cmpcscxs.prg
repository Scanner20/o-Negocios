     ...
     LfStkMinF  = 0
	     ...
	     ** Stock por sedes **
	     FOR K=1 TO nSedes
		     ...
		     SELE CALM
		     SEEK LsCodMat
		     SCAN WHILE CodMat=LsCodMat FOR ALMA.Codalm=vSedes(K,1)
		          LfStkIni = LfStkIni + round(CapStkAlm(SubAlm,CodMat,m.dfch1),1)
		     ...
		     IF LfStkMin>0
		        REPLACE &CstkDia. WITH ROUND(LfStkIni*DiaXMes/LfStkMin,0)
		        vSedes(K,3)=[*]
		     ENDIF
       ...
       **ENDIF
         ...
         ** Consumos acumulados
		 replace CsmAcm with CAPCSMOMES(CodMat,m.dFchIni,m.dFch1)		        	
	     **--------------------**
	     LsMsgErr = []
	     IF !lHayStkMin
	        LsMsgErr=LsMsgErr +[No Stk.Min. ]
	     ENDIF
	     IF CATG.TmpRep+CATG.StkSeg>DT AND lHayStkMin
	        LcFlag =[*]
	        DO CASE
