...
m.vendedor = space(4)
m.vendedor2 = space(4)
m.FchDoc	= CTOD("01/01/"+STR(_ANO,4,0)) && DATE()
m.FchDoc2	= DATE()
M.FchVto	= CTOD("01/01/"+STR(_ANO,4,0)) && DATE()
m.FchVto2	= DATE()
m.TipRep = 1
m.DiaAdd = 0
m.NombreVen=''
...
*XFOR = "FchDoc>=m.FchDoc AND FchDoc<=m.fchdoc2 .AND. !(FlgEst=[A].AND.ImpTot=0)"
*!*	SET STEP ON 
XFOR = "FchDoc>=m.FchDoc AND FchDoc<=m.fchdoc2 and FchVto+m.DiaAdd>=m.FchVto AND FchVto+m.DiaAdd<=m.fchVto2 .AND. (FlgEst=[P] AND SdoDoc>=.1)"
	...
	IF GDOC->FLGEST#[A]
		...
		*!* buscamos documentos de cancelacion *!*
		...
		IF m.SdoDoc > 0.01   && << OJO <<
			...
			REPLACE FchVto WITH IIF(GDOC.CodDoc='LETR',GDOC->FchVto+8,GDOC.FchVto)
