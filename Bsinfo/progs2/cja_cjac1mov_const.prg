...
DIMENSION xCodCta(40),xCodMon(40),xNroRef(40),xCodAux(40),xcoddiv(40)
TsCodDiv1= [01]
XsCodOpe = ""
...
STORE "" TO XsNroAst,XsNotAst,XnCodMon,XfTpoCmb,XiNroVou,XsGloAst,XsCaux
STORE "" TO XsCtaCja,XfImpChq,XsNroChq,XsGirado,XsCodAux,XsAuxil,XsCodDo1
STORE 0 TO XfImpCh1,XfImpCh2,XfPorcen
STORE {} TO XdFchAst,XdFchPed
XdFchAst = DATE()
XdFchPed = XdFchAst
XfTpoCmb = 3.5
...
*!*					ENDIF
*!*					DO MOVMover
*!*					nAccion = f1_alert('Desea modificar el voucher?','SI_O_NO')
*!*					UltTecla = 0
*!*				ENDIF
*!*				IF nAccion = 1
*!*					DO MOVEdita
...
@ 19,00 SAY "CONCEPTO...:" COLOR SCHEME 11
...
PROCEDURE MovNoDoc
******************
*!*	Posicionamos puntero en la tabla de operaciones
SELECT OPER
IF !XsCodOpe = OPER.CodOpe
	SEEK XsCodOpe
	IF ! FOUND()
...
@ 04,10 SAY "N§ CUENTA : "+CTAS->NroCta+" "+CTAS->RefBco   COLOR SCHEME 11
@ 4 ,68 SAY TpoCmb PICT "9999.9999"   COLOR SCHEME 11
@ 05,68 SAY FchPed   COLOR SCHEME 11
@ 5,12  SAY XsNroChq  COLOR SCHEME 11
@ 5 ,38 SAY Auxil  COLOR SCHEME 11
IF VMOV->CodMon = 1
	XnCodMon = 1
	@ 18,14 say "S/. "  COLOR SCHEME 11
ELSE
	XnCodMon = 2
	@ 18,14 say "US$ "  COLOR SCHEME 11
ENDIF
@  8,01 clear TO 10,78  COLOR SCHEME 11
@ 13,01 CLEAR TO 16,78  COLOR SCHEME 11
@  8,01 FILL TO 10,78  COLOR SCHEME 11
@ 13,01 FILL TO 16,78  COLOR SCHEME 11
LinAct1 = 8
LinAct2 = 13
IF VMOV->FlgESt = "A"
	@ LinAct1,0 say []
...
SELECT VMOV
@ 18 ,18 SAY VMOV->ImpChq PICT "99,999,999,999.99"   COLOR SCHEME 11
@ 19 ,14 SAY VMOV->NotAst  COLOR SCHEME 11
RETURN
...
DO WHILE .NOT. INLIST(UltTecla,F10,CtrlW,Escape_)
	DO CASE
		...
		CASE I = 5
			...
			@ 04,10 SAY "N§ CUENTA : "+NroCta+" "+RefBco  COLOR SCHEME 11
			IF CTAS->CodMon = 1
				XnCodMon = 1
				@ 18,14 say "S/. "  COLOR SCHEME 11
			ELSE
		...
		CASE I = 10
			...
			@ 18 ,18 SAY XfImpChq PICT "99,999,999,999.99"  COLOR SCHEME 11
			IF LASTKEY() = Escape_
				UltTecla = Arriba
			ELSE
				UltTecla = Enter
		...
		CASE I = 12
			...
			READ
			UltTecla = LastKey()
			@ 19, 14 SAY XsNotAst PICT "@!"  COLOR SCHEME 11
		CASE I = 14
			SAVE SCREEN
...
@ NumLin,09 SAY vNroAst(NumEle) PICT "@R 999-9999999"    COLOR SCHEME 11
@ NumLin,24 SAY vNotAst(NumEle) PICT "@S40"   COLOR SCHEME 11
...
*!*						@ NumLin,09  GET LsNroAst PICT "999-999999"
					IF INLIST(PROV.TpoDoc,'FAC','BOL')
						@ NumLin,09  GET LsNroAst PICT "@R 999-9999999"
					ELSE 
...
*!*					LsNroAst = PADR(LsNroAst,GDOC.NroDoc)
				IF INLIST(PROV.TpoDoc,'FAC','BOL')
					@ NumLin,09  SAY LsNroAst PICT "@R 999-9999999"  COLOR SCHEME 11
				ELSE
			...
			IF ! CHKNROAST()
				...
				LOOP
			ENDIF
			@ NumLin,20 SAY LsNotAst PICT "@S40"  COLOR SCHEME 11
		CASE i = 3
			@ NumLin,61 SAY IIF(XnCodMon=1,"S/.","US$")  COLOR SCHEME 11
...
ELSE
	...
	*!*		@ 21,22 SAY AUXI->NomAux PICT "@S50"
	...
	IF EMPTY(XsNotAst) .OR. MaxEle = 1
		XsNotAst = LsNotAst
		@ 20,14 SAY XsNotAst  COLOR SCHEME 11
	ENDIF
ENDIF	
SET ORDER TO VMOV01
RETURN .T.
...
REPLACE RMOV.CodCco  WITH XsCodDo1
...
*!*			IF Import = 0 .OR. EliItm = "*"   
		...
		@ LinAct,6 SAY "|"  
		@ LinAct,0  SAY _PRN4
		@ LinAct,07 SAY LsGloDoc    PICT "@S52" font 'Courier New',5
		@ LinAct,0  SAY _PRN2
		@ LinAct,39+2 SAY TRIM(CodCta)   font 'Courier New',6
		@ LinAct,45+3 SAY NroDoc   font 'Courier New',6
		@ LinAct,55+4 SAY NroRef   font 'Courier New',6
		IF TpoMov = "D"
...
@ 10,6  SAY "| "+PADR("CUENTA No: "+CTAS->NROCTA,86)+"³"
...
LsNotAst = NomCli
xCodCta(NumEle) = IIF(CodMon=1,TDOC.Cta12_MN,TDOC.Cta12_ME)
xCodMon(NumEle) = CodMon
xNroRef(NumEle) = ""
...
SEEK GDOC->CodDoc+GDOC->NroDoc+PADR("I/C",LEN(CodDoc))+PADR(XsCodOpe+XsNroAst,LEN(NroDoc))
IF !FOUND()
	RETURN
ENDIF
IF !RLOCK()
