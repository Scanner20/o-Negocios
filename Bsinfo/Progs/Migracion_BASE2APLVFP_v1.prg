...
DO WHILE !EOF() 
	IF !'\CIA'$JUSTPATH(cPath)
		...
		LOOP
	ENDIF
	LsRutaCia=JUSTPATH(cPath)
	LsCodCia=JUSTSTEM(LsRutaCia)
	DirAct=LsRutaCia
	...
	DO WHILE ! EMPTY(LEFT(cArcAct,8))
		LsTabla=ADDBS(DirAct)+cArcAct
		IF !INDBC(JUSTSTEM(cArcAct),'TABLE')
			ADD TABLE (LsTabla)
	...
	SCAN WHILE JUSTPATH(cPath)=LsRutaCia
		...
		LsRutaBDAno=STRTRAN(RTRIM(cPath),LsCodCia+'\C'+LsAno,'P'+SUBSTR(LsCodCia,4,3))+LsAno
