...
MakeView_V_MATERIALES_X_ALMACEN(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_CULTIVOS_X_LOTE...])
MakeView_V_CULTIVOS_X_LOTE(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_MATERIALES_X_ALMACEN_2...])
MakeView_V_MATERIALES_X_ALMACEN_2(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_MATERIALES_SIN_ALMACEN...])
MakeView_V_MATERIALES_SIN_ALMACEN(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_MATERIALES_X_ALMACEN_3...])
MakeView_V_MATERIALES_X_ALMACEN_3(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_MOVIMIENTOS_ALMACEN...])
MakeView_V_MOVIMIENTOS_ALMACEN(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_MATERIALES_X_LOTE...])
MakeView_V_MATERIALES_X_LOTE(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_REPORTE_GUIA...])
MakeView_V_REPORTE_GUIA(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_NOTAS_BALANCE...])
MakeView_V_NOTAS_BALANCE(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_FORMULAS_DESC...])
MakeView_V_FORMULAS_DESC(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_ALMCATGE...])
MakeView_V_ALMCATGE(PsNomcia, PsNomBdAno)
DisplayStatus([Creating view V_NOTAS_EEFF_PRESUPUESTO...])
MakeView_V_NOTAS_EEFF_PRESUPUESTO(PsNomcia, PsNomBdAno)
...
IF !INDBC('cpiculti','TABLE')
	RETURN
ENDIF
IF !INDBC('Cpicuxlt','TABLE')
	RETURN
ENDIF
...
CREATE SQL VIEW "V_NOTAS_EEFF_PRESUPUESTO" ; 
   AS SELECT Cbdnpres.nota, Cbdnpres.codcta, Cbdmctas.nomcta, Cbdnpres.signo, Cbdnpres.forma, Cbdnpres.ctapre, ;
    ...
    LEFT OUTER JOIN &PsNomBDAno.!CBDMCTAS  ON  Cbdnpres.codcta = Cbdmctas.codcta  ON  Cbdtpres.rubro = Cbdnpres.rubro AND ;
...
FUNCTION AlterTable_CBDMCTAS
SELECT 0
USE CBDMCTAS EXCLUSIVE
IF VARTYPE(CTADRV)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	CtaDRV C(1) 
ENDIF
IF VARTYPE(USERCREA)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	USERCREA C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(USERMODI)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	USERMODI C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(USERELIM)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	USERELIM C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(FCHCREA)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	FCHCREA T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(FCHMODI)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	FCHMODI T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(FCHELIM)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	FCHELIM T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(TAG_MIGRA)='U'
	ALTER TABLE CBDMCTAS ADD COLUMN	TAG_MIGRA C(4)
ENDIF

FUNCTION AlterTable_CBDRMOVM
SELECT 0
USE CBDRMOVM EXCLUSIVE
IF VARTYPE(USERCREA)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	USERCREA C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(USERMODI)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	USERMODI C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(USERELIM)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	USERELIM C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(FCHCREA)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	FCHCREA T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(FCHMODI)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	FCHMODI T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(FCHELIM)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	FCHELIM T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(TAG_MIGRA)='U'
	ALTER TABLE CBDRMOVM ADD COLUMN	TAG_MIGRA C(4)
ENDIF
RETURN

FUNCTION AlterTable_CBDVMOVM
SELECT 0
USE CBDVMOVM EXCLUSIVE
IF VARTYPE(USERCREA)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	USERCREA C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(USERMODI)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	USERMODI C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(USERELIM)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	USERELIM C(10) NOT NULL DEFAULT IIF(VARTYPE(gsusuario)#"C",ALLTRIM(SUBSTR(SYS(0),AT("#",SYS(0))+1)),gsusuario)
ENDIF
IF VARTYPE(FCHCREA)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	FCHCREA T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(FCHMODI)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	FCHMODI T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(FCHELIM)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	FCHELIM T NOT NULL DEFAULT DATETIME()
ENDIF
IF VARTYPE(TAG_MIGRA)='U'
	ALTER TABLE CBDVMOVM ADD COLUMN	TAG_MIGRA C(4)
ENDIF
RETURN

FUNCTION AlterTable_CBDACMCT
RETURN
