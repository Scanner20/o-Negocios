*!*	DO TEST_CSV_GRE IN test_csv_varios WITH "G/R ","T00500000001" 
*!*	DO TEST_CSV_FACT IN test_csv_varios WITH "CARGO","FACT","E001000000004" 
*!*	DO TEST_XML_GRE IN test_csv_varios WITH "G/R ","0001000001059"
*!*	DO TEST_JSON_GRE IN Test_CSV_Varios WITH "G/R ","EG07000000097"
PROCEDURE Test_XML_GRE
	*!*	Aca ingreso mi código
	...
	IF FOUND()
		...
		ENDIF
	ELSE
		MESSAGEBOX("No existe el documento: " + PsCodDoc + " " + PsNroDoc + " EN CTRA", 0+16, "Verficar parametros del test")
		RETURN
	ENDIF
	...
	DO Vta_Genara_Archivos_SEE_SFS_GRE_V1 WITH oData1, oData2, oData3, NULL, LsRuta1, LsRuta2
	USE IN CTRA
	USE IN DTRA
	USE IN GUIA
	RETURN
...
PROCEDURE TEST_JSON_GRE
	*!*	Aca ingreso mi código
	...
	LsRuta1 = goentpub.tspath_ose_gre
	LsRuta2 = goentpub.tspath_ose_csv
	DO vta_prggeneraarchivos_nubefact WITH oData1, oData2, oData3, NULL, LsRuta1, LsRuta2
	USE IN CTRA
	USE IN DTRA
	USE IN GUIA
	RETURN
...
do vta_genera_txt_csv-efact_v1 WITH oData1,odata2,odata3,null,LsRuta1,LsRuta2
USE IN CTRA
USE IN DTRA
USE IN GUIA
RETURN

PROCEDURE Test_JSON_Fact_Bole
	PARAMETERS PsTpoDoc, PsCodDoc, PsNroDoc
	IF USED("GDoc")
		USE IN GDoc
	ENDIF
	gocfgvta.odatadm.abrirtabla("ABRIR","CCBRGDOC","GDOC","GDOC01") && TPODOC+CODDOC+NRODOC
	IF USED("Item")
		USE IN Item
	ENDIF
	gocfgvta.odatadm.abrirtabla("ABRIR","VTARITEM","ITEM","ITEM01") && TPODOC+CODDOC+NRODOC
	IF USED("Guia")
		USE IN Guia
	ENDIF
	gocfgvta.odatadm.abrirtabla("ABRIR","vtavguia","GUIA","VGUI03") && CODFAC+NROFAC
	IF USED("RCuo")
		USE IN RCuo
	ENDIF
	gocfgvta.odatadm.abrirtabla("ABRIR","VTARCUOT","RCUO","FACT") && TPOREF+NROREF+NROREF
	SELECT GDOC
	SET RELATION TO CODDOC + NRODOC INTO ITEM ADDITIVE
	SET RELATION TO TPODOC+ CODDOC + NRODOC INTO RCUO ADDITIVE
	SEEK PsTpoDoc+PsCodDoc+PsNroDoc
	IF FOUND()
		SCATTER NAME oData1
		SELECT ITEM
		IF ITEM.CodDoc+ITEM.NroDoc=PsCodDoc+PsNroDoc
			oData2 = gocfgvta.odatadm.genobjdatos("ITEM",[CodDoc+NroDoc="]+PsCodDoc+PsNroDoc+["])
			SELECT GUIA
			SEEK PsCodDoc + PsNroDoc
			IF FOUND()
				** SCATTER NAME oData3 && Solo cuando es un solo registro
				oData3 = gocfgvta.odatadm.genobjdatos("GUIA",[CodFac+NroFac="]+PsCodDoc+PsNroDoc+["])
			ELSE
	*!*				MESSAGEBOX("NO EXISTE DOCUMENTO "+PsCodDoc+" "+PsNroDoc+"EN GUIA" ,0+16,"Verficar parametros del test")
	*!*				RETURN
				oData3 = NULL
			ENDIF
		ENDIF
		IF RCUO.TpoRef+RCUO.CodRef+RCUO.NroRef=PsTpoDoc+PsCodDoc+PsNroDoc
			oData4 = gocfgvta.odatadm.genobjdatos("RCUO",[TpoRef+CodRef+NroRef="]+PsTpoDoc+PsCodDoc+PsNroDoc+["])
		ELSE
			oData4 = NULL 
		ENDIF
		gocfgvta.L_VTARCUOT = .T.
	ELSE
		MESSAGEBOX("NO EXISTE DOCUMENTO "+PsCodDoc+" "+PsNroDoc+" EN GDOC",0+16,"Verficar parametros del test")
		RETURN
	ENDIF
	*!*	IF EMPTY(gocfgvta.RutaSEE_SFS)
	*!*		gocfgvta.Ruta_Factura_SEE_SFS()
	...
	*!*	LsRuta=gocfgvta.RutaSEE_SFS
	...
	LsRuta2 = goentpub.tspath_ose_csv
	DO vta_prggeneraarchivos_nubefact WITH oData1, oData2, oData3, oData4, LsRuta1, LsRuta2
	IF USED("GDOC")
		USE IN GDOC
	ENDIF
	IF USED("ITEM")
